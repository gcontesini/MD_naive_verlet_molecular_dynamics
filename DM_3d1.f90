...
real*8               :: par, temp, dt, vol, den, tini, dqm
...
open(8,file='energia1.dat',status='unknown')
...
call lenjon(nat,nx,ny,nz,par,rx,ry,rz,fx,fy,fz,Epot)
...
print*, "--------------------- PROGRAMA DinMol3D -------------------------"
print*, "Dinamica  molecular 3D de um sistema de  NAT atomos posicionados "
...
call verletmod(nat,nx,ny,nz,dt,par,rx,ry,rz,vx,vy,vz,fx,fy,fz,fax,fay,faz,Epot)
...
dqm = 0.
do j=1,nat
  dqm = dqm +(rx(j)-rx0(j))**2 + (ry(j)-ry0(j))**2 + (rz(j)-rz0(j))**2 
enddo
...
write(*,'(a,f12.4)')   ' Energia Total Media <T>/nat           =', (Emmed+Epmed+Ecmed)/dfloat(nat)
print*

!pause !para plataforma windows

! -----------------------------------------------------------------------------
end program dinamol
! -----------------------------------------------------------------------------

subroutine lenjon(nat,nx,ny,nz,par,rx,ry,rz,fx,fy,fz,Epot)
implicit none

integer, intent(in) :: nat, nx, ny, nz
real*8, intent(in)  :: rx(nat), ry(nat), rz(nat), par
real*8, intent(out) :: fx(nat), fy(nat), fz(nat), Epot
...
subroutine verletmod(nat,nx,ny,nz,dt,par,rx,ry,rz,vx,vy,vz,fx,fy,fz,fax,fay,faz,Epot)
implicit none

! ----------------- Variaveis da Subrotina -------------------------------
! Externas
integer,intent(in)   :: nat, nx, ny, nz
real*8, intent(in)   :: fax(nat), fay(nat), faz(nat), par, dt
real*8, intent(inout):: rx(nat), ry(nat), rz(nat), vx(nat), vy(nat), vz(nat)
real*8, intent(inout):: fx(nat), fy(nat), fz(nat)
